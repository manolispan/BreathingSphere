...
PRIVATE
PUBLIC  :: bessj_amos, bessy_amos, bes3, bes4, cbesh, cbesi, cbesj,             & 
     &     cbesk, cbesy, cairy, cbiry, gamln, goodold_bessel , bessj, bessy,      &
     &     bessj0, bessy0, spbessel , qbessj,qbessy,bes2


CONTAINS
subroutine BESSJ_amos(ARG,CJ,CDJ,LMAX,LMAXD)
  implicit none
  integer                     :: lmaxd,LMAX
  complex(kind=DP),intent(in) :: arg
  complex(kind=DP),intent(out):: cj(lmaxd),cdj(lmaxd)
!
  real(kind=DP),parameter   :: fnu =0.5_dp
  integer,parameter         :: kode = 1
  real(kind=DP)             :: PI 
  integer                   :: nzj,ierrj,i
  complex(kind=DP)          :: fact,cj0
  pi = 4.0_dp*datan(1.0_dp)

  fact = sqrt(pi/2.0_dp/arg)
  CALL cbesj(arg, fnu, kode,LMAXD, cj, nzj, ierrj)
  cj0 = cj(1)*fact
  do i=1,lmax   
   cj(i) = fact*cj(i+1)
  end do
  if (ierrj.ne.0) write(6,"('Bessel j problem :',I3)") ierrj  
  cdj(1) = cj0 - 1.0_dp/arg*cj(1)
  do i=2,lmax 
  cdj(i) = cj(i-1) - float(i)/arg*cj(i)    ! (n+1) -> i
  end do 

end subroutine BESSJ_amos
subroutine BESSY_amos(ARG,CY,CDY,LMAX,LMAXD)
  implicit none
  integer                     :: lmaxd,LMAX
  complex(kind=DP),intent(in) :: arg
  complex(kind=DP),intent(out):: cy(lmaxd),cdy(lmaxd)
!
  real(kind=DP),parameter   :: fnu =0.5_dp
  integer,parameter         :: kode = 1
  real(kind=DP)             :: PI    
  integer                   :: nzy,ierry,i
  complex(kind=DP)          :: fact,cy0
  pi = 4.0_dp*datan(1.0_dp)
  fact = sqrt(pi/2.0_dp/arg) 
  CALL cbesy(arg, fnu, kode,      LMAXD, cy, nzy, ierry)
  cy0 = cy(1)*fact
  do i=1,lmax   
   cy(i) = fact*cy(i+1)
  end do 
  if (ierry.ne.0) write(6,"('Bessel y problem :',I3)") ierry
  cdy(1) = cy0 - 1.0_dp/arg*cy(1)
  do i=2,lmax 
  cdy(i) = cy(i-1) - float(i)/arg*cy(i)    
  end do 
end subroutine BESSY_amos



SUBROUTINE bes3(cj,cy,ch,arg,lmax1d,lmax,lj,ly,lh) 
 implicit none

! bes3 was added for compatibility with multem code

  integer                     :: lmax1d,lmax,lmaxd
  complex(kind=DP),intent(in) :: arg
  complex(kind=DP),intent(out):: cj(lmax1d),ch(lmax1d),cy(lmax1d)
  logical                     :: lj,ly,lh  
!
  real(kind=DP),parameter   :: fnu =0.5_dp
  integer,parameter         :: kode = 1
  real(kind=DP)             :: PI ! = 3.14159265358979324_DP
  integer                   :: nzh,nzj,ierrh,ierrj,i,ierry,nzy
  complex(kind=DP)          :: fact
  pi = 4.0_dp*atan(1.0_dp)
  fact = sqrt(pi/2.d0/arg) 
  lmaxd = lmax1d - 1
  if (lmax.gt.lmaxd) then
      write(6,*) 'Error dimensions in Bes3'
  end if
  if (lj) then
    CALL cbesj(arg, fnu, kode,      LMAX1D, cj, nzj, ierrj)
     cj(1:lmax1D) = fact*cj(1:lmax1D)
     if (ierrj.ne.0) write(6,"('Bessel j problem :',I3)") ierrj 
  end if

  if (ly) then
    CALL cbesy(arg, fnu, kode,      LMAX1D, cy, nzy, ierry)
     cy(1:lmax1D) = fact*cy(1:lmax1D)
     if (ierry.ne.0) write(6,"('Bessel y problem :',I3)") ierry 
  end if      
  if (lh) then
    
    CALL cbesh(arg, fnu, kode, 1  , lmax1D, ch, nzh, ierrh)
     ch(1:lmax1D) = fact*ch(1:lmax1D)
     if (ierrh.ne.0) write(6,"('Bessel h problem :',I3)") ierrh 
  end if 
...
!-----------------------------------------------------------------------
  ...
  l1 = 0
  l2 = 30
  btol = tol * (ABS(REAL(bsum)) + ABS(dIMAG(bsum)))
  atol = tol
  pp = 1.0_dp
  ias = 0
  ibs = 0
...
DO  i = 1, kk
  ...
  t1 = t1 - cone
END DO
IF (REAL(p1, KIND=dp) == 0.0_dp .AND. dIMAG(p1) == 0.0_dp) THEN
...
DO  i = 2, n
  ...
  IF (REAL(pt, KIND=dp) == 0.0_dp .AND. dIMAG(pt) == 0.0_dp) THEN
...
!--------------------------------------------------------------------
!C======================================================================
      SUBROUTINE QBESSJ(X,Y,DY,LMAX,LMAXI)
      use params, ONLY: LMAXD,DP,QP,CQP
      IMPLICIT NONE
!C     -----------------------------------------------------------------
!C
!C ..  PARAMETER STATEMENTS ..
!C
      INTEGER,PARAMETER:: LMAXID=50*LMAXD
!C
!C ..  SCALAR ARGUMENTS ..
!C
      INTEGER::    LMAX,LMAXI
...
!C ..  ARRAY ARGUMENTS ..
!C
      COMPLEX(CQP)  :: Y(LMAXD),DY(LMAXD)
!C
!C ..  LOCAL SCALARS ..
!C
      INTEGER :: LL,I
      COMPLEX(CQP)  :: XX,Z0,Y0,Y1,YI1,YI
!C
!C ..  LOCAL ARRAYS ..
!C
      COMPLEX(CQP)  :: Z(LMAXID)
!C
...
!C
      LL=LMAX+LMAXI
      IF(LL.GT.LMAXID) STOP 'ERROR IN BESSJ: LMAXI TOO HIGH'
     
      XX=1.0_qp/X
      Z(LL)=1.0_qp/(FLOAT(2*LL+1)*XX)
      DO 5 I=LL-1,1,-1
         Z(I)=1.0_qp/(FLOAT(2*I+1)*XX-Z(I+1))
    5 CONTINUE
      ...
      DO 10 I=2,LMAX
         YI1=Y(I-1)
         YI=YI1*Z(I)
         DY(I)=YI1-FLOAT(I)*YI*XX
         Y(I)=YI
   10 CONTINUE
      RETURN
   END subroutine qbessj
!C======================================================================
      SUBROUTINE QBESSY(X,Y,DY,LMAX)
      use params, ONLY: LMAXD,DP,QP,CQP
      IMPLICIT NONE
!C     -----------------------------------------------------------------
!C
...
!C
      ...
      Y1=-C*X2-S*X1
      Y(1)=Y1
      Y(2)=(-3.0_qp*X3+X1)*C-3.0_qp*X2*S
      LMAX1=LMAX-1
      DO 5 I=2,LMAX1
    5     Y(I+1)=DFLOAT(2*I+1)*X1*Y(I)-Y(I-1)
      DY(1)=-X1*(C+Y1)
      DO 10 I=2,LMAX
  10       DY(I)=Y(I-1)-DFLOAT(I)*X1*Y(I)
      RETURN
      END subroutine QBESSY

      SUBROUTINE BESSJ(X,Y,DY,LMAX,LMAXI)
      use params, ONLY: LMAXD,DP
      IMPLICIT NONE
!C     -----------------------------------------------------------------
!C
!C ..  PARAMETER STATEMENTS ..
...
!C ..  ARRAY ARGUMENTS ..
!C
      COMPLEX(kind=DP)  :: Y(LMAXD),DY(LMAXD)
!C
!C ..  LOCAL SCALARS ..
!C
      INTEGER :: LL,I
      COMPLEX(kind=DP)  :: XX,Z0,Y0,Y1,YI1,YI
!C
!C ..  LOCAL ARRAYS ..
!C
      COMPLEX(kind=DP)  :: Z(50*LMAXD)  ! LMAXID
 !C
!C ..  INTRINSIC FUNCTIONS ..
!C
      INTRINSIC COS
...
!C
      ...
      IF(LL.GT.LMAXID) STOP 'ERROR IN BESSJ: LMAXI TOO HIGH'
      XX=1D0/X
      Z(LL)=1D0/(DFLOAT(2*LL+1)*XX)
      DO 5 I=LL-1,1,-1
         Z(I)=1D0/(DFLOAT(2*I+1)*XX-Z(I+1))
    5 CONTINUE
      Z0=1D0/(XX-Z(1))
      Y0=Z0*COS(X)*XX
      Y1=Y0*Z(1)
      ...
      DO 10 I=2,LMAX
         YI1=Y(I-1)
         YI=YI1*Z(I)
         DY(I)=YI1-DFLOAT(I)*YI*XX
         Y(I)=YI
   10 CONTINUE
      RETURN
      END subroutine bessj
!C======================================================================
      SUBROUTINE BESSY(X,Y,DY,LMAX)
      use params, ONLY: LMAXD,DP
      IMPLICIT NONE
!C     -----------------------------------------------------------------
!C
...
!C
      INTEGER I,LMAX1
      COMPLEX(kind=DP)  :: C,S,X1,X2,X3,Y1
!C
!C ..  INTRINSIC FUNCTIONS ..
!C
      INTRINSIC COS,SIN
...
!C
  ...
  10       DY(I)=Y(I-1)-DFLOAT(I)*X1*Y(I)
      ...
      SUBROUTINE BESSJ0(X,Y,DY,LMAXD,LMAX,LMAXI)
! Y(1) == Y(l=1), the l=0 is missing
...
!C ..  ARRAY ARGUMENTS ..
!C
      COMPLEX(kind=DP)  :: Y(LMAXD+1),DY(LMAXD+1)
!C
!C ..  LOCAL SCALARS ..
!C
      INTEGER :: LL,I
      COMPLEX(kind=DP)  :: XX,Z0,Y0,Y1,YI1,YI,s,c
!C
!C ..  LOCAL ARRAYS ..
!C
      COMPLEX(kind=DP)  :: Z(50*LMAXD)  ! LMAXID
...
!C
    ...
    5 CONTINUE
      ...
      DO 10 I=2,LMAX
         ...
         Y(I)=YI
   10 CONTINUE
! shift index up and add the l=0
!
!
         do i=lmax,1,-1
            y(i+1) = y(i)
...
!C======================================================================
      SUBROUTINE BESSY0(X,Y,DY,LMAXD,LMAX)
      use params, ONLY: DP
      IMPLICIT NONE
!C     -----------------------------------------------------------------
!C
...
!C
      if (lmax.gt.lmaxd+1) then 
        write(6,*) 'BESSJ0 increase lmaxd', lmax,lmaxd
      end if
      C=COS(X)
      S=SIN(X)
      X1=1D0/X
...
!
     RETURN
      ...
      SUBROUTINE SPBESSEL(J,Y,H,X,LMAXD,LMAX,LMAXI)
! Driver to replace BESSEL sub 20 Sep 2012
! This subroutine uses downward recursion for Bessel and upward
! recursion for Newman functions. For large arguments hankel
! is calculated by an upward recursion. 
! 
! It is intented to be a general purpose bessel, for small arguments
! the old bessel is supposed to work better.
! A new input LMAXI is required that sets the LMAX that the downwards
! recursion starts. A big value (lmaxi=50) should work for most cases
! a better estimate is given below. 
! One problem is that lmaxi should not be changed during a calculation
! since this produces noise in the results 
! High LMAXI should always work but increases computation time. 
!
! The solution here uses a fixed LMAXI but checks and gives 
! a warning 
!
      ...
      implicit none  
        integer :: LMAXD
        COMPLEX(kind=DP) :: X
        COMPLEX(kind=DP) :: H(LMAXD+1),DH(LMAXD+1),h1(lmaxd+1)
        integer          :: LMAX,LMAXI,LMAXI0,LL,L11
        real(kind=dp)    :: argmax
        complex(kind=dp) :: ci
        COMPLEX(kind=DP)  :: Y(LMAXD+1),DY(LMAXD+1),J(LMAXD+1),DJ(LMAXD+1)

        ci =cmplx(0.0d0,1.0d0,kind=dp)
! This is the correct way to set lmaxi     
        ARGMAX=ABS(X)
        ARGMAX=MAX(ABS(ARGMAX),FLOAT(LMAX))
        LMAXI0=(ARGMAX+4.0*ARGMAX**(1.0d0/3.0d0)                             &
      &                             +1.2d0*SQRT(ARGMAX))
        if (LMAXI.lt.LMAXI0) write(6,*) ' SPBESSEL Warning increase LMAXI ' 

           CALL BESSJ0(x,J,DJ,LMAXD,LMAX,LMAXI)
           CALL BESSY0(x,Y,DY,LMAXD,LMAX)
          ...
          DO LL=1,LMAX+1
           H(LL)=J(LL)+CI*Y(LL)
           DH(LL) = DJ(LL) + CI*DY(LL)
          END DO !LL LOOP           
          if (abs(x).gt.20.d0) then

          write(6,*) 'Assymptotic expansion for Hankel no derivative !'
          H1(1)=EXP(CI*x)/(CI*x)
          H1(2)=-EXP(CI*x)*(1.D0+CI/x)/x
          DO LL=3,LMAX
             L11=LL-1
             H1(LL)=FLOAT(2*L11-1)*H1(LL-1)/x-H1(LL-2)
             
             if (abs(h(ll)-h1(ll)).gt.1.d-12) then 
                write(6,*) 'Asymptotic expansion for Hankel fix derivative'
                h(ll) = h1(ll)
             end if 
          END DO !LL LOOP
     ...
     end SUBROUTINE SPBESSEL
        


   
!--------------------------------------------------------------------
      SUBROUTINE GOODOLD_BESSEL(BJ,Y,H,ARG,LMX,LMAX,LJ,LY,LH,LCALL) 
      use params, only: DP
      use constants
      IMPLICIT NONE 
!C     ------------------------------------------------------------------  
...
!C  
      ...
      INTEGER,PARAMETER:: NDIMP1=NDIM+1
...
!C ..  ARRAY ARGUMENTS  ..  
!C  
      COMPLEX(kind=DP):: BJ(LMX+1),H(LMX+1),Y(LMX+1)  
!C   
!C ..  LOCAL SCALARS  ..  
!C  
      INTEGER    ::       L,LMSAVE,N,NCNW,NMAX  
      REAL(kind=DP)  ::   CPJ,CPY,FJ,FY,ONE,SUM,W,W2  
      COMPLEX(kind=DP) :: RES,T0,T1,T2,TARG,TT1  
!C      
!C ..  LOCAL ARRAYS  ..  
!C  
      REAL(kind=DP)   :: CNWJ(20,NDIMP1),CNWY(20,NDIMP1)  
      COMPLEX(kind=DP):: TN(20), ZN(NDIMP2)  
!C       
...
!      DATA CI/ (0.D0,1.D0)/  
      DATA W2,ONE,NMAX/12.D0,1.D0,20/  
!C     ------------------------------------------------------------------  
!C  
      IF (.NOT.LCALL) THEN  
         IF (LMAX.GT.25) THEN  
...
30      CONTINUE  
        ...
        T0 = CONE  
!        TARG = -ZN(3)*0.25D0/W  
        TARG = -ARG*ARG*0.25D0/W  ! Changed by GGANTZOU 16/03/06
        T1 = CTWO*TARG - CONE  
        TN(1) = T0  
        TN(2) = T1  
        TT1 = T1 + T1  
...
30    CONTINUE  
      SUM = SUM/RHO  
      RETURN  
      END  subroutine CNWF01

     SUBROUTINE BES2(BJ,Y,H,ARG,LMX,LMAX,LJ,LY,LH)
      use params,only:dp
      IMPLICIT NONE
!C     -----------------------------------------------------------------
!C     THIS SUBROUTINE CREATES SPHERICAL BESSEL FUNCTION OF FIRST ORDER 
!C     FOR HIGH MANGITUDES OF THE ARGUMENT
!C     ------------------------------------------------------------------
!C
!C     .. SCALAR ARGUMENTS ..
!C
...
!C     .. LOCAL ARRAYS ..
!C
      COMPLEX(dp) CSJ(0:LMAX),CDJ(0:LMAX),CSY(0:LMAX),CDY(0:LMAX)
!C
!C     .. DATA STATEMENTS ..
!C
      DATA CI/(0.D0,1.D0)/
!C     -----------------------------------------------------------------
!C      
      CALL CSPHJY(LMAX,ARG,NM,CSJ,CDJ,CSY,CDY)
      IF(NM.LT.LMAX) STOP 'ERROR IN BES2: LMAX TOO HIGH'
      IF(LJ) THEN
      DO 10 I=1,LMAX+1 !Changed by GGANTZOU 26/06/06
      BJ(I)=CSJ(I-1)
10    CONTINUE
             ENDIF
      IF(LY) THEN
      DO 20 I=1,LMAX+1 !Changed by GGANTZOU 26/06/06
      Y(I)=CSY(I-1)
20    CONTINUE
             ENDIF
      IF(LH) THEN
      DO 30 I=1,LMAX+1 !Changed by GGANTZOU 26/06/06
      H(I)=CSJ(I-1)+CI*CSY(I-1)
30    CONTINUE
             ENDIF
      RETURN
   END subroutine BES2
...
!C       ==========================================================
!C
!        IMPLICIT COMPLEX(kind=DP) (C,Z)
        implicit none
        real(kind=DP):: A0
        integer:: N,NM
        complex(kind=DP):: CSJ(0:N),CDJ(0:N),CSY(0:N),CDY(0:N)
        complex(kind=DP):: Z
!
         complex(kind=DP):: CSA,CSB,CF0,CF1,CF,CS
        integer:: K,M!,MSTA1,MSTA2
